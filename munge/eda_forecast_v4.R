...
h=6
x_ts <- df_full %>% filter(channel_uid=="c149b6fafe123318b768e2e1c716abeb")%>% pull(engagements)
model_arima <- forecast::auto.arima(x_ts)
metric_name='Engagements'
plot_forecast(x_ts, model_arima,model_name = 'Auto Arima' ,metric_name =metric_name,h = h)


model_ema <- TTR::EMA(x_ts)
plot_forecast(x_ts, model_ema,model_name = 'EMA' ,metric_name = metric_name ,h = h)

model_ets=forecast::ets(x_ts)
plot_forecast(x_ts,model_ets,model_name = 'ETS' ,metric_name = metric_name,h = h)

model_theta=forecast::thetaf(x_ts)
plot_forecast(x_ts,model_theta,model_name = 'Theta' ,metric_name = metric_name,h = h)

x_xts=xts::as.xts(x_ts,order.by=df_temp$published_date)
x_ts_ts <- ts(as.numeric(x_xts), frequency = 365, start = c(as.numeric(format(start(x_xts), "%Y")), as.numeric(format(start(x_xts), "%j"))))
...
plot_forecast(x_ts,model_nnet,model_name = 'Neural net autoregressive',metric_name = metric_name,h = h)
